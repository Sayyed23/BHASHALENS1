  ...
  Future<void> _processSimplify() async {
    ...
    try {
      ...
      final result = await geminiService.translateText(
        ...
        sourceLanguage: _targetComplexity,
      );
      ...
      if (mounted) {
        setState(() {
          ...
          _explanation =
              'Simplified version of your text using Gemini Strict Mode.';
          _isLoading = false;
        });
      }
    } catch (e) {
        ...
        child: Column(
          children: [
              ...
              children: [
                ...
                Expanded(
                  child: DropdownButtonFormField<String>(
                    ...
                    items: const [
                      DropdownMenuItem(
                          value: 'simple', child: Text("Very Simple")),
                      DropdownMenuItem(
                          value: 'moderate', child: Text("Moderate")),
                      DropdownMenuItem(
                          value: 'complex', child: Text("Professional")),
                    ],
                    onChanged: (val) =>
                        setState(() => _targetComplexity = val!),
                  ),
                ),
              ],
            ),
            ...
            if (_simplifiedText.isNotEmpty) ...[
              ...
              _buildResultCard(
                  "Simplified Meaning", _simplifiedText, Icons.auto_awesome),
              if (_explanation != null) ...[
                const SizedBox(height: 20),
                _buildResultCard(
                    "Educational Insight", _explanation!, Icons.lightbulb),
              ],
            ],
          ],
        ),
