...
class SimplifyModePage extends StatefulWidget {
  ...
  const SimplifyModePage({super.key, this.initialText});
...
class _SimplifyModePageState extends State<SimplifyModePage>
  ...
  void initState() {
    ...
    if (widget.initialText != null && widget.initialText!.isNotEmpty) {
      ...
      Future.delayed(Duration.zero, _processSimplify);
    }
  }
  ...
  Future<void> _processSimplify() async {
    ...
    try {
      ...
      if (isOffline) {
        result = await offlineService.simplifyAsMap(
          text,
      ...
      if (mounted) {
        if (extractedText.isNotEmpty && extractedText != 'No text detected') {
          setState(() {
            ...
            _isProcessing = false;
            _tabController.index = 2; // Switch to Text tab to show result
          });
          _processSimplify();
        } else {
          setState(() => _isProcessing = false);
                  ...
                  if (_simplifyData == null) ...[
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton.icon(
                        onPressed: _isProcessing ? null : _processSimplify,
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 16),
              Text(
                voiceService.isListening ? "Listening..." : "Tap to start speaking",
                style: TextStyle(
                  color: voiceService.isListening ? primaryBlue : Colors.white,
                  fontWeight: FontWeight.bold,
                ),
              ),
              const SizedBox(height: 8),
              const Text(
                "Explain a concept or read a sentence",
    ...
    return Column(
      ...
      children: [
        ...
        Container(
          ...
          decoration: BoxDecoration(
            color: const Color(0xFFEBF3FF), // Light blue like mockup but content is dark
            borderRadius: BorderRadius.circular(24),
          ),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
        ...
        if (_simplifyData!['explanation'] != null)
          _buildInfoSection("Insight", _simplifyData!['explanation'], Icons.info_outline),
        
        const SizedBox(height: 16),
        
        // Key Points
        if (_simplifyData!['key_points'] != null && (_simplifyData!['key_points'] as List).isNotEmpty)
          _buildKeyPointsSection(_simplifyData!['key_points']),
