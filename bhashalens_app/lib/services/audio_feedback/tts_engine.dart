...
abstract class TtsEngine {
  ...
  /// Set speech rate (0.0 to 2.0, default 1.0)
...
class FlutterTtsEngine implements TtsEngine {
  ...
  Future<void> resume() async {
    try {
      ...
      // We need to handle this differently based on platform
      if (Platform.isIOS) {
        // iOS supports pause/resume
        await _flutterTts.pause();
      } else {
        // For Android, we might need to restart from where we left off
        // This is a limitation of the current flutter_tts implementation
        _updateState(TtsState.continued);
      }
    } catch (e) {
      if (kDebugMode) {
  ...
  Future<void> setVoice(String voiceName) async {
    try {
      ...
      final voice = voices.firstWhere(
        (v) => v.name == voiceName,
        orElse: () => voices.isNotEmpty ? voices.first : const Voice(name: 'default', locale: 'en-US'),
      );
  ...
  String detectLanguage(String text) {
    ...
    } else if (RegExp(r'[àâäéèêëïîôöùûüÿç]', caseSensitive: false).hasMatch(text)) {
      return 'fr-FR'; // French
